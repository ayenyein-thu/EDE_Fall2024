...
```{r project.setup, message=FALSE, errors=FALSE, echo=TRUE}
#Import libraries
library(tidyverse)
library(lubridate)
library(here)
...
```{r, message=FALSE, warning=FALSE, echo=FALSE, fig.height=3, fig.cap="2018 Monthly Ozone Levels", fig.align='left'}
#Create boxplot of the monthly distribution of Ozone levels in 2018
Ozone_2018 <- epa_data %>% 
  filter(Year=="2018") %>% 
...
```{r, message=FALSE, warning=FALSE, echo=FALSE, fig.height=3, fig.cap="2019 Monthly Ozone Levels", fig.align='left'}
#Create boxplot of the monthly distribution of Ozone levels in 2019
Ozone_2019 <- epa_data %>% 
  filter(Year=="2019") %>% 
  ggplot(aes(x=factor(Month, levels = 1:12, labels = month.abb),
             y=Ozone)) +
