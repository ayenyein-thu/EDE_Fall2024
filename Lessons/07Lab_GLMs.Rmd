...
```{r}
#Exercise 1:
plot_1 <- ggplot(EPAair, aes(x = Ozone, 
                             fill=as.factor(Year))) + 
...
ggplot(EPAair, aes(x=Ozone, y=PM2.5)) + 
  ...
  geom_text(x=100,y=75,label=expression("PM,2.5=0.38*Ozone+15.6"))
...
Testing <- lm(
  data=EPAair,PM2.5~Ozone + mean_Lat + mean_Lng 
)
